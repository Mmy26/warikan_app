...
describe("GroupController", () => {
  ...
  describe("addGroup", () => {
    ...
    it("バリデーションエラー: グループ名は必須", () => {
      ...
      expect(res.send).toBeCalledWith(["グループ名は必須です"]);
    });

    it("バリデーションエラー: メンバーは２人以上必須", () => {
      ...
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.send).toBeCalledWith(["メンバーは2人以上必要です"]);
    });
    ...
    it("バリデーションエラー: 同じ名前のメンバーは登録できない", () => {
      ...
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.send).toBeCalledWith(["メンバー名が重複しています"]);
    });
  });
});
